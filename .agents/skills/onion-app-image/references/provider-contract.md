...
- `reference_images`：按Logo、IP、风格/构图、字体、真实UI的已确认顺序传入，单次最多8张且总计不超过64MB；每张只包文件名、MIME和base64内容；
...
2. 明确预计生成数并取得当前付费确认。
3. 逐job调`generation_kie_image`。网络超时或响应不确定时使用同一幂等Key回查，不创建新Key。
4. 成功响应必须包含`output_id`、`mime_type`、`byte_count`、`sha256`、供应商`task_id`和短时下载地址。立即下载到清单输出路径并核对哈希；短时地址不进入长期交付。
5. 原请求已失败时停止并保留错误状态。只有用户明确修改Prompt、参考图或候选意图并同意新生成，才建立新版本Key。
